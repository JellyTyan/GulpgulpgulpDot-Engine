...
namespace GulpgulpgulpdotPlugins
{
    public static class Main
    {
        ...
        private static readonly Assembly CoreApiAssembly = typeof(global::Gulpgulpgulpdot.GulpgulpgulpdotObject).Assembly;
        ...
        private static unsafe gulpgulpgulpdot_bool InitializeFromEngine(IntPtr gulpgulpgulpdotDllHandle, gulpgulpgulpdot_bool editorHint,
            ...
            IntPtr unmanagedCallbacks, int unmanagedCallbacksSize)
        {
            try
            {
                ...
                _dllImportResolver = new GulpgulpgulpdotDllImportResolver(gulpgulpgulpdotDllHandle).OnResolveDllImport;
                ...
                if (_editorHint)
                {
                    _editorApiAssembly = Assembly.Load("GulpgulpgulpdotSharpEditor");
        ...
        {
            public unsafe delegate* unmanaged<char*, gulpgulpgulpdot_string*, gulpgulpgulpdot_bool> LoadProjectAssemblyCallback;
            public unsafe delegate* unmanaged<char*, IntPtr, int, IntPtr> LoadToolsAssemblyCallback;
            public unsafe delegate* unmanaged<gulpgulpgulpdot_bool> UnloadProjectPluginCallback;
        }

        [UnmanagedCallersOnly]
        private static unsafe gulpgulpgulpdot_bool LoadProjectAssembly(char* nAssemblyPath, gulpgulpgulpdot_string* outLoadedAssemblyPath)
        {
            try
            {
                if (_projectLoadContext != null)
                    return gulpgulpgulpdot_bool.True; // Already loaded
            ...
            IntPtr unmanagedCallbacks, int unmanagedCallbacksSize)
        {
            try
            {
                ...
                if (_editorApiAssembly == null)
                    throw new InvalidOperationException("The Gulpgulpgulpdot editor API assembly is not loaded.");

                var (assembly, _) = LoadPlugin(assemblyPath, isCollectible: false);

                NativeLibrary.SetDllImportResolver(assembly, _dllImportResolver!);

                var method = assembly.GetType("GulpgulpgulpdotTools.GulpgulpgulpdotSharpEditor")?
                    .GetMethod("InternalCreateInstance",
                        BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public);

                if (method == null)
                {
                    throw new MissingMethodException("GulpgulpgulpdotTools.GulpgulpgulpdotSharpEditor",
        ...
        [UnmanagedCallersOnly]
        private static gulpgulpgulpdot_bool UnloadProjectPlugin()
        {
            try
            {
                return UnloadPlugin(ref _projectLoadContext).ToGulpgulpgulpdotBool();
            }
            catch (Exception e)
            {
                Console.Error.WriteLine(e);
        ...
        private static bool UnloadPlugin(ref PluginLoadContextWrapper? pluginLoadContext)
        {
            try
            {
                ...
                while (pluginLoadContext.IsAlive)
                {
                    ...
                    if (!takingTooLong && elapsedTimeMs >= 200)
                    {
                        ...
                        // TODO: How to log from GulpgulpgulpdotPlugins? (delegate pointer?)
            ...
            {
                // TODO: How to log exceptions from GulpgulpgulpdotPlugins? (delegate pointer?)
                Console.Error.WriteLine(e);
                return false;
            }
        }
