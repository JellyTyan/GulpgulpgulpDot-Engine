...
namespace Gulpgulpgulpdot.NativeInterop
{
    internal static class ExceptionUtils
    {
        ...
        private static void SendToScriptDebugger(Exception e)
        {
            ...
            using gulpgulpgulpdot_string nFile = Marshaling.ConvertStringToNative(file);
            using gulpgulpgulpdot_string nFunc = Marshaling.ConvertStringToNative(func);
            using gulpgulpgulpdot_string nErrorMsg = Marshaling.ConvertStringToNative(errorMsg);
            using gulpgulpgulpdot_string nExcMsg = Marshaling.ConvertStringToNative(excMsg.ToString());

            using DebuggingUtils.gulpgulpgulpdot_stack_info_vector stackInfoVector = default;
            ...
            unsafe
            {
                ...
                {
                    DebuggingUtils.gulpgulpgulpdot_stack_info* stackInfo = &stackInfoVector.Elements[i];
                ...
                NativeFuncs.gulpgulpgulpdotsharp_internal_script_debugger_send_error(nFunc, nFile, line,
                    nErrorMsg, nExcMsg, gulpgulpgulpdot_error_handler_type.ERR_HANDLER_ERROR, stackInfoVector);
            }
        }
        ...
        public static void LogException(Exception e)
        {
            try
            {
                if (NativeFuncs.gulpgulpgulpdotsharp_internal_script_debugger_is_active().ToBool())
                {
        ...
        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(gulpgulpgulpdot_string_name method, IntPtr instance, gulpgulpgulpdot_variant** args, int argCount, gulpgulpgulpdot_variant_call_error error)
        {
            if (error.Error != gulpgulpgulpdot_variant_call_error_error.GULPGULPGULPDOT_CALL_ERROR_CALL_OK)
            {
                using gulpgulpgulpdot_variant instanceVariant = VariantUtils.CreateFromGulpgulpgulpdotObjectPtr(instance);
        ...
        [Conditional("DEBUG")]
        public unsafe static void DebugCheckCallError(in gulpgulpgulpdot_callable callable, gulpgulpgulpdot_variant** args, int argCount, gulpgulpgulpdot_variant_call_error error)
        {
            if (error.Error != gulpgulpgulpdot_variant_call_error_error.GULPGULPGULPDOT_CALL_ERROR_CALL_OK)
            {
                using gulpgulpgulpdot_variant callableVariant = VariantUtils.CreateFromCallableTakingOwnershipOfDisposableValue(callable);
        ...
        private unsafe static string GetCallErrorWhere(ref gulpgulpgulpdot_variant_call_error error, gulpgulpgulpdot_string_name method, gulpgulpgulpdot_variant* instance, gulpgulpgulpdot_variant** args, int argCount)
        {
            string? methodstr = null;
            string basestr = GetVariantTypeName(instance);

            if (method == GulpgulpgulpdotObject.MethodName.Call || (basestr == "Gulpgulpgulpdot.TreeItem" && method == TreeItem.MethodName.CallRecursive))
            {
                if (argCount >= 1)
                {
                    methodstr = VariantUtils.ConvertToString(*args[0]);
                    if (error.Error == gulpgulpgulpdot_variant_call_error_error.GULPGULPGULPDOT_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT)
                    {
                        error.Argument += 1;
                    }
                }
        ...
        private unsafe static string GetCallErrorMessage(gulpgulpgulpdot_variant_call_error error, string where, gulpgulpgulpdot_variant** args)
        {
            switch (error.Error)
            {
                case gulpgulpgulpdot_variant_call_error_error.GULPGULPGULPDOT_CALL_ERROR_CALL_ERROR_INVALID_ARGUMENT:
                {
                ...
                case gulpgulpgulpdot_variant_call_error_error.GULPGULPGULPDOT_CALL_ERROR_CALL_ERROR_TOO_MANY_ARGUMENTS:
                case gulpgulpgulpdot_variant_call_error_error.GULPGULPGULPDOT_CALL_ERROR_CALL_ERROR_TOO_FEW_ARGUMENTS:
                    return $"Invalid call to {where}. Expected {error.Expected} argument(s).";
                case gulpgulpgulpdot_variant_call_error_error.GULPGULPGULPDOT_CALL_ERROR_CALL_ERROR_INVALID_METHOD:
                    return $"Invalid call. Nonexistent {where}.";
                case gulpgulpgulpdot_variant_call_error_error.GULPGULPGULPDOT_CALL_ERROR_CALL_ERROR_INSTANCE_IS_NULL:
                    return $"Attempt to call {where} on a null instance.";
                case gulpgulpgulpdot_variant_call_error_error.GULPGULPGULPDOT_CALL_ERROR_CALL_ERROR_METHOD_NOT_CONST:
        ...
        private unsafe static string GetVariantTypeName(gulpgulpgulpdot_variant* variant)
        {
            if (variant->Type == Variant.Type.Object)
            {
                GulpgulpgulpdotObject obj = VariantUtils.ConvertToGulpgulpgulpdotObject(*variant);
                if (obj == null)
                {
                    return "null instance";
                }
                else if (!GulpgulpgulpdotObject.IsInstanceValid(obj))
                {
