...
namespace GulpgulpgulpdotTools.ProjectEditor
{
    ...
    {
        [GeneratedRegex(@"\s*'\$\(GulpgulpgulpdotTargetPlatform\)'\s*==\s*'(?<platform>[A-z]+)'\s*", RegexOptions.IgnoreCase)]
        private static partial Regex GulpgulpgulpdotTargetPlatformConditionRegex();
        ...
        public static void UpgradeProjectIfNeeded(MSBuildProject project, string projectName)
        {
            ...
            EnsureGulpgulpgulpdotSdkIsUpToDate(project);
        ...
        public static void EnsureGulpgulpgulpdotSdkIsUpToDate(MSBuildProject project)
        {
            var root = project.Root;
            string gulpgulpgulpdotSdkAttrValue = ProjectGenerator.GulpgulpgulpdotSdkAttrValue;

            if (!string.IsNullOrEmpty(root.Sdk) &&
                root.Sdk.Trim().Equals(gulpgulpgulpdotSdkAttrValue, StringComparison.OrdinalIgnoreCase))
                return;
        ...
        {
            var root = project.Root;
            string minTfmValue = ProjectGenerator.GulpgulpgulpdotMinimumRequiredTfm;
            ...
            foreach (var propertyGroup in root.PropertyGroups)
            {
                ...
                if (groupHasCondition && !ConditionMatchesGulpgulpgulpdotPlatform(propertyGroup.Condition))
                {
                    continue;
                }
                ...
                foreach (var property in propertyGroup.Properties)
                {
                    ...
                    if (propertyHasCondition && !ConditionMatchesGulpgulpgulpdotPlatform(property.Condition))
                    {
                        continue;
                    }
            ...
            foreach (var property in propertiesToChange)
            {
                ...
                // the minimum required by Gulpgulpgulpdot, we don't want to remove the conditional
            ...
            static bool ConditionMatchesGulpgulpgulpdotPlatform(string condition)
            {
                // Check if the condition is checking the 'GulpgulpgulpdotTargetPlatform' for one of the
                // Gulpgulpgulpdot platforms with built-in support in the Gulpgulpgulpdot.NET.Sdk.
                var match = GulpgulpgulpdotTargetPlatformConditionRegex().Match(condition);
                if (match.Success)
                {
