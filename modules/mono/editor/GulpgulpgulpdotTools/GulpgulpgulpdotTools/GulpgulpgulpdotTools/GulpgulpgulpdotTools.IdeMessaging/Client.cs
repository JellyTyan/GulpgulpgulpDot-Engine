...
namespace GulpgulpgulpdotTools.IdeMessaging
{
    ...
    public sealed class Client : IDisposable
    {
        ...
        private GulpgulpgulpdotIdeMetadata gulpgulpgulpdotIdeMetadata;
        private readonly FileSystemWatcher fsWatcher;

        public string GulpgulpgulpdotEditorExecutablePath => gulpgulpgulpdotIdeMetadata.EditorExecutablePath;
        ...
        public Client(string identity, string gulpgulpgulpdotProjectDir, IMessageHandler messageHandler, ILogger logger)
        {
            this.identity = identity;
            this.messageHandler = messageHandler;
            this.logger = logger;

            string projectMetadataDir = Path.Combine(gulpgulpgulpdotProjectDir, ".gulpgulpgulpdot", "mono", "metadata");
            // FileSystemWatcher requires an existing directory
            if (!Directory.Exists(projectMetadataDir))
            {
                // Check if the non hidden version exists
                string nonHiddenProjectMetadataDir = Path.Combine(gulpgulpgulpdotProjectDir, "gulpgulpgulpdot", "mono", "metadata");
            ...
            MetaFilePath = Path.Combine(projectMetadataDir, GulpgulpgulpdotIdeMetadata.DefaultFileName);

            fsWatcher = new FileSystemWatcher(projectMetadataDir, GulpgulpgulpdotIdeMetadata.DefaultFileName);
        }
        ...
        private async void OnMetaFileChanged(object sender, FileSystemEventArgs e)
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (metadata != null && metadata != gulpgulpgulpdotIdeMetadata)
                {
                    gulpgulpgulpdotIdeMetadata = metadata.Value;
        ...
        private GulpgulpgulpdotIdeMetadata? ReadMetadataFile()
        {
            ...
            using (var reader = new StreamReader(fileStream))
            {
                ...
                return new GulpgulpgulpdotIdeMetadata(port, editorExecutablePath);
            }
        }
        ...
        private async Task ConnectToServer()
        {
            ...
            try
            {
                logger.LogInfo("Connecting to Gulpgulpgulpdot Ide Server");

                await tcpClient.ConnectAsync(IPAddress.Loopback, gulpgulpgulpdotIdeMetadata.Port);

                logger.LogInfo("Connection open with Gulpgulpgulpdot Ide Server");
            ...
            catch (SocketException e)
            {
                if (e.SocketErrorCode == SocketError.ConnectionRefused)
                    logger.LogError("The connection to the Gulpgulpgulpdot Ide Server was refused");
                else
                    throw;
            }
        }
        ...
        public async void Start()
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (!File.Exists(MetaFilePath))
                {
                    logger.LogInfo("There is no Gulpgulpgulpdot Ide Server running");
                    return;
                }
                ...
                {
                    logger.LogError("Failed to read Gulpgulpgulpdot Ide metadata file");
                }
            }
        }
            ...
            where TResponse : Response, new()
        {
            if (!IsConnected)
            {
                logger.LogError("Cannot write request. Not connected to the Gulpgulpgulpdot Ide Server.");
                return null;
            }
