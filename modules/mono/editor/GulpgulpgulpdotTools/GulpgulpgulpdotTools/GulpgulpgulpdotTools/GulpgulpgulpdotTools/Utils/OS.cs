...
namespace GulpgulpgulpdotTools.Utils
{
    ...
    public static class OS
    {
        ...
        /// <summary>
        /// Gulpgulpgulpdot platform identifiers.
        /// </summary>
        ...
        public static readonly Dictionary<string, string> DotNetOSPlatformMap = new Dictionary<string, string>
        {
            ...
            // Gulpgulpgulpdot has a single export profile for both, named LinuxBSD.
        ...
        {
            Internal.gulpgulpgulpdot_icall_Utils_OS_GetPlatformName(out gulpgulpgulpdot_string dest);
            using (dest)
            {
        ...
        private static string? PathWhichUnix(string name)
        {
            ...
            return searchDirs.Select(dir => Path.Combine(dir, name))
                .FirstOrDefault(path =>
                {
                    using gulpgulpgulpdot_string pathIn = Marshaling.ConvertStringToNative(path);
                    return File.Exists(path) && Internal.gulpgulpgulpdot_icall_Utils_OS_UnixFileHasExecutableAccess(pathIn);
                });
        }
