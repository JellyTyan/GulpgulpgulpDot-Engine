...
using GulpgulpgulpdotTools.BuildLogger;
...
namespace GulpgulpgulpdotTools.Build
{
    public static class BuildSystem
    {
        ...
        {
            var verbosityLevel =
                editorSettings.GetSetting(GulpgulpgulpdotSharpEditor.Settings.VerbosityLevel).As<VerbosityLevelId>();
            arguments.Add("-v");
            ...
            if ((bool)editorSettings.GetSetting(GulpgulpgulpdotSharpEditor.Settings.NoConsoleLogging))
                arguments.Add("-noconlog");
        }

        private static void AddLoggerArgument(BuildInfo buildInfo, Collection<string> arguments)
        {
            string buildLoggerPath = Path.Combine(Internals.GulpgulpgulpdotSharpDirs.DataEditorToolsDir,
                "GulpgulpgulpdotTools.BuildLogger.dll");

            arguments.Add(
                $"-l:{typeof(GulpgulpgulpdotBuildLogger).FullName},{buildLoggerPath};{buildInfo.LogsDirPath}");
        }

        private static void AddBinaryLogArgument(BuildInfo buildInfo, Collection<string> arguments,
            EditorSettings editorSettings)
        {
            if (!(bool)editorSettings.GetSetting(GulpgulpgulpdotSharpEditor.Settings.CreateBinaryLog))
                return;
        ...
        {
            var baseDylib = $"{GulpgulpgulpdotSharpDirs.ProjectAssemblyName}.dylib";
            var baseSym = $"{GulpgulpgulpdotSharpDirs.ProjectAssemblyName}.framework.dSYM";
