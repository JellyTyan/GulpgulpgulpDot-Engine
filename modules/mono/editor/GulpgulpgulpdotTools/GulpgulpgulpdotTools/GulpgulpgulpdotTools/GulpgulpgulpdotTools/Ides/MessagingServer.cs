...
using GulpgulpgulpdotTools.IdeMessaging;
...
namespace GulpgulpgulpdotTools.Ides
{
    public sealed class MessagingServer : IDisposable
    {
        ...
        public MessagingServer(string editorExecutablePath, string projectMetadataDir, ILogger logger)
        {
            ...
            _metaFilePath = Path.Combine(projectMetadataDir, GulpgulpgulpdotIdeMetadata.DefaultFileName);

            // Make sure the directory exists
            Directory.CreateDirectory(projectMetadataDir);

            // The Gulpgulpgulpdot editor's file system thread can keep the file open for writing, so we are forced to allow write sharing...
            ...
            where TResponse : Response, new()
        {
            using (await _peersSem.UseAsync())
            {
                ...
                {
                    _logger.LogError("Cannot write request. No client connected to the Gulpgulpgulpdot Ide Server.");
                    return;
                }
        ...
        private class ServerMessageHandler : IMessageHandler
        {
            private static void DispatchToMainThread(Action action)
            {
                ...
                Gulpgulpgulpdot.Dispatcher.SynchronizationContext.Post(d, null);
            }
            ...
            private static Task<Response> HandleDebugPlay(DebugPlayRequest request)
            {
                DispatchToMainThread(() =>
                {
                    ...
                    GulpgulpgulpdotSharpEditor.Instance.SkipBuildBeforePlaying = !(request.BuildBeforePlaying ?? true);
                    ...
                    Environment.SetEnvironmentVariable("GULPGULPGULPDOT_MONO_DEBUGGER_AGENT",
                    ...
                    Environment.SetEnvironmentVariable("GULPGULPGULPDOT_MONO_DEBUGGER_AGENT", "");
                    GulpgulpgulpdotSharpEditor.Instance.SkipBuildBeforePlaying = false;
                });
            ...
            private static async Task<Response> HandleCodeCompletionRequest(CodeCompletionRequest request)
            {
                ...
                await Gulpgulpgulpdot.Engine.GetMainLoop().ToSignal(Gulpgulpgulpdot.Engine.GetMainLoop(), "process_frame");
