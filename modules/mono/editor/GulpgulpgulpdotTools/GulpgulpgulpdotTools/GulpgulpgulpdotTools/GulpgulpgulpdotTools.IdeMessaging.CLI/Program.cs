...
namespace GulpgulpgulpdotTools.IdeMessaging.CLI
{
    internal static class Program
    {
        ...
        private static async Task<int> StartAsync(string[] args, Stream inputStream, Stream outputStream)
        {
            ...
            try
            {
                ...
                string gulpgulpgulpdotProjectDir = args[0];

                if (!Directory.Exists(gulpgulpgulpdotProjectDir))
                {
                    Logger.LogError($"The specified Gulpgulpgulpdot project directory does not exist: {gulpgulpgulpdotProjectDir}");
                    return 1;
                }

                var forwarder = new ForwarderMessageHandler(outputWriter);

                using (var fwdClient = new Client("VisualStudioCode", gulpgulpgulpdotProjectDir, forwarder, Logger))
                {
                    fwdClient.Start();
