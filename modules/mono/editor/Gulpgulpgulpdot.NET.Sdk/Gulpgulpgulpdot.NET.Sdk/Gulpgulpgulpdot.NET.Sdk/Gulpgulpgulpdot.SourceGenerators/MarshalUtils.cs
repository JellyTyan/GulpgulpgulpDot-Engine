...
namespace Gulpgulpgulpdot.SourceGenerators
{
    internal static class MarshalUtils
    {
        ...
        {
            public INamedTypeSymbol GulpgulpgulpdotObjectType { get; }
            ...
            public TypeCache(Compilation compilation)
            {
                ...
                GulpgulpgulpdotObjectType = GetTypeByMetadataNameOrThrow(GulpgulpgulpdotClasses.GulpgulpgulpdotObject);
            }
        }
        ...
        public static VariantType? ConvertMarshalTypeToVariantType(MarshalType marshalType)
            => marshalType switch
            {
                ...
                MarshalType.GulpgulpgulpdotObjectOrDerivedArray => VariantType.Array,
                ...
                MarshalType.GulpgulpgulpdotObjectOrDerived => VariantType.Object,
                MarshalType.StringName => VariantType.StringName,
                MarshalType.NodePath => VariantType.NodePath,
                MarshalType.Rid => VariantType.Rid,
                MarshalType.GulpgulpgulpdotDictionary => VariantType.Dictionary,
                MarshalType.GulpgulpgulpdotArray => VariantType.Array,
                MarshalType.GulpgulpgulpdotGenericDictionary => VariantType.Dictionary,
                MarshalType.GulpgulpgulpdotGenericArray => VariantType.Array,
                _ => null
            };
        ...
        public static MarshalType? ConvertManagedTypeToMarshalType(ITypeSymbol type, TypeCache typeCache)
        {
            ...
            switch (specialType)
            {
                ...
                default:
                {
                    ...
                    {
                        if (type.ContainingAssembly?.Name == "GulpgulpgulpdotSharp" &&
                            type.ContainingNamespace?.Name == "Gulpgulpgulpdot")
                        {
                            return type switch
                            {
                    ...
                    else if (typeKind == TypeKind.Array)
                    {
                        ...
                        if (elementType.SimpleDerivesFrom(typeCache.GulpgulpgulpdotObjectType))
                            return MarshalType.GulpgulpgulpdotObjectOrDerivedArray;

                        if (elementType.ContainingAssembly?.Name == "GulpgulpgulpdotSharp" &&
                            elementType.ContainingNamespace?.Name == "Gulpgulpgulpdot")
                        {
                            switch (elementType)
                            {
                                case { Name: "Vector2" }:
                    ...
                    {
                        if (type.SimpleDerivesFrom(typeCache.GulpgulpgulpdotObjectType))
                            return MarshalType.GulpgulpgulpdotObjectOrDerived;

                        if (type.ContainingAssembly?.Name == "GulpgulpgulpdotSharp")
                        {
                            switch (type.ContainingNamespace?.Name)
                            {
                                case "Gulpgulpgulpdot":
                                    return type switch
                                    {
                                ...
                                case "Collections"
                                    when type.ContainingNamespace?.FullQualifiedNameOmitGlobal() == "Gulpgulpgulpdot.Collections":
                                    return type switch
                                    {
                                        { Name: "Dictionary" } =>
                                            type is INamedTypeSymbol { IsGenericType: false } ?
                                                MarshalType.GulpgulpgulpdotDictionary :
                                                MarshalType.GulpgulpgulpdotGenericDictionary,
                                        { Name: "Array" } =>
                                            type is INamedTypeSymbol { IsGenericType: false } ?
                                                MarshalType.GulpgulpgulpdotArray :
                                                MarshalType.GulpgulpgulpdotGenericArray,
                                        _ => null
                                    };
                            }
                        }
        ...
        private const string VariantUtils = "global::Gulpgulpgulpdot.NativeInterop.VariantUtils";
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            ...
            {
                // We need a special case for GulpgulpgulpdotObjectOrDerived[], because it's not supported by VariantUtils.ConvertTo<T>
                MarshalType.GulpgulpgulpdotObjectOrDerivedArray =>
                    source.Append(VariantUtils, ".ConvertToSystemArrayOfGulpgulpgulpdotObject<",
                        ((IArrayTypeSymbol)typeSymbol).ElementType.FullQualifiedNameIncludeGlobal(), ">(",
                        inputExpr, ")"),
                // We need a special case for generic Gulpgulpgulpdot collections and GulpgulpgulpdotObjectOrDerived[], because VariantUtils.ConvertTo<T> is slower
                MarshalType.GulpgulpgulpdotGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for GulpgulpgulpdotObjectOrDerived[], because it's not supported by VariantUtils.CreateFrom<T>
                MarshalType.GulpgulpgulpdotObjectOrDerivedArray =>
                    source.Append(VariantUtils, ".CreateFromSystemArrayOfGulpgulpgulpdotObject(", inputExpr, ")"),
                // We need a special case for generic Gulpgulpgulpdot collections and GulpgulpgulpdotObjectOrDerived[], because VariantUtils.CreateFrom<T> is slower
                MarshalType.GulpgulpgulpdotGenericDictionary =>
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for GulpgulpgulpdotObjectOrDerived[], because it's not supported by Variant.As<T>
                MarshalType.GulpgulpgulpdotObjectOrDerivedArray =>
                    source.Append(inputExpr, ".AsGulpgulpgulpdotObjectArray<",
                        ((IArrayTypeSymbol)typeSymbol).ElementType.FullQualifiedNameIncludeGlobal(), ">()"),
                // We need a special case for generic Gulpgulpgulpdot collections and GulpgulpgulpdotObjectOrDerived[], because Variant.As<T> is slower
                MarshalType.GulpgulpgulpdotGenericDictionary =>
                    source.Append(inputExpr, ".AsGulpgulpgulpdotDictionary<",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[0].FullQualifiedNameIncludeGlobal(), ", ",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[1].FullQualifiedNameIncludeGlobal(), ">()"),
                MarshalType.GulpgulpgulpdotGenericArray =>
                    source.Append(inputExpr, ".AsGulpgulpgulpdotArray<",
                        ((INamedTypeSymbol)typeSymbol).TypeArguments[0].FullQualifiedNameIncludeGlobal(), ">()"),
            ...
            string inputExpr, ITypeSymbol typeSymbol, MarshalType marshalType)
        {
            return marshalType switch
            {
                // We need a special case for GulpgulpgulpdotObjectOrDerived[], because it's not supported by Variant.From<T>
                MarshalType.GulpgulpgulpdotObjectOrDerivedArray =>
                    source.Append("global::Gulpgulpgulpdot.Variant.CreateFrom(", inputExpr, ")"),
                // We need a special case for generic Gulpgulpgulpdot collections, because Variant.From<T> is slower
                MarshalType.GulpgulpgulpdotGenericDictionary or MarshalType.GulpgulpgulpdotGenericArray =>
                    source.Append("global::Gulpgulpgulpdot.Variant.CreateFrom(", inputExpr, ")"),
                _ => source.Append("global::Gulpgulpgulpdot.Variant.From<",
                    typeSymbol.FullQualifiedNameIncludeGlobal(), ">(", inputExpr, ")")
            };
        }
    }
