...
namespace Gulpgulpgulpdot.SourceGenerators
{
    ...
    public class ScriptPropertyDefValGenerator : ISourceGenerator
    {
        ...
        {
            if (context.IsGulpgulpgulpdotSourceGeneratorDisabled("ScriptPropertyDefVal"))
                return;
        ...
        )
        {
            ...
            bool isNode = symbol.InheritsFrom("GulpgulpgulpdotSharp", GulpgulpgulpdotClasses.Node);
            ...
            var exportedProperties = members
                ...
                .Where(s => s.GetAttributes()
                    .Any(a => a.AttributeClass?.IsGulpgulpgulpdotExportAttribute() ?? false))
                .ToArray();
            ...
            foreach (var property in exportedProperties)
            {
                ...
                // Two possible ways: reflection or turn RestoreGulpgulpgulpdotObjectData into a constructor overload.
                // Ignore properties without a getter, without a setter or with an init-only setter.
                // Gulpgulpgulpdot properties must be both readable and writable.
            ...
            foreach (var field in exportedFields)
            {
                ...
                // Ignore properties without a getter or without a setter. Gulpgulpgulpdot properties must be both readable and writable.
                if (field.IsReadOnly)
                {
                    context.ReportDiagnostic(Diagnostic.Create(
                        Common.ExportedMemberIsReadOnlyRule,
            ...
            // Generate GetGulpgulpgulpdotExportedProperties
            ...
            if (exportedMembers.Count > 0)
            {
                ...
                const string DictionaryType =
                    "global::System.Collections.Generic.Dictionary<global::Gulpgulpgulpdot.StringName, global::Gulpgulpgulpdot.Variant>";

                source.Append("#if TOOLS\n");

                source.Append("    /// <summary>\n")
                    .Append("    /// Get the default values for all properties declared in this class.\n")
                    .Append("    /// This method is used by Gulpgulpgulpdot to determine the value that will be\n")
                ...
                source.Append(" GetGulpgulpgulpdotPropertyDefaultValues()\n    {\n");
        ...
        {
            if (marshalType == MarshalType.GulpgulpgulpdotObjectOrDerived)
            {
                return memberType.InheritsFrom("GulpgulpgulpdotSharp", GulpgulpgulpdotClasses.Node);
            }
            if (marshalType == MarshalType.GulpgulpgulpdotObjectOrDerivedArray)
            {
                var elementType = ((IArrayTypeSymbol)memberType).ElementType;
                return elementType.InheritsFrom("GulpgulpgulpdotSharp", GulpgulpgulpdotClasses.Node);
            }
            if (memberType is INamedTypeSymbol { IsGenericType: true } genericType)
            {
                return genericType.TypeArguments
                    .Any(static typeArgument
                        => typeArgument.InheritsFrom("GulpgulpgulpdotSharp", GulpgulpgulpdotClasses.Node));
            }
