"""
Load this file to your GDB session to enable pretty-printing of some Gulpgulpgulpdot C++ types.

GDB command: `source misc/utility/gulpgulpgulpdot_gdb_pretty_print.py`.
...
...
    {
        "description": "Load custom pretty-printers for Gulpgulpgulpdot types.",
        "text": "source ${workspaceFolder}/misc/utility/gulpgulpgulpdot_gdb_pretty_print.py"
    }
]
```
...
# Printer for Gulpgulpgulpdot StringName variables.
class GulpgulpgulpdotStringNamePrinter:
...
# Printer for Gulpgulpgulpdot String variables.
class GulpgulpgulpdotStringPrinter:
    def __init__(self, value):
        self.value = value
...
# Printer for Gulpgulpgulpdot Vector variables.
class GulpgulpgulpdotVectorPrinter:
    def __init__(self, value):
        self.value = value
...
def lookup_pretty_printer(value):
    if value.type.name == "StringName":
        return GulpgulpgulpdotStringNamePrinter(value)
    if value.type.name == "String":
        return GulpgulpgulpdotStringPrinter(value)
    if value.type.name and VECTOR_REGEX.match(value.type.name):
        return GulpgulpgulpdotVectorPrinter(value)
    return None
